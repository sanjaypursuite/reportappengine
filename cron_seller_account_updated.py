...
query = '(SELECT *FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY tb1.user_id ORDER BY tb1.updated_date DESC) AS RowNo FROM `indigo-lotus-154020.php.seller_account_updated` tb1) x WHERE x.RowNo = 1)'
