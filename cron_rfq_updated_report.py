...
query = '(SELECT *FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY tb1.rfq_id ORDER BY tb1.updated_date DESC) AS RowNo FROM `indigo-lotus-154020.nodejs.rfq_updated_report_data` tb1) x WHERE x.RowNo = 1)'
job_config = bigquery.QueryJobConfig()
job_config.allow_large_results = True
dest_dataset_ref = client.dataset('cron')
dest_table_ref = dest_dataset_ref.table('rfq_updated_report_data_etl')
