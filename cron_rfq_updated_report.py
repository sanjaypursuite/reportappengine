...
query = '(SELECT *FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY tb1.rfq_id ORDER BY tb1.updated_date DESC) AS RowNo FROM `indigo-lotus-154020.nodejs.rfq_updated_report` tb1) x WHERE x.RowNo = 1)'
