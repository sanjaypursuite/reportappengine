...
query = '(SELECT *FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY tb1.quotation_id ORDER BY tb1.updated_date DESC) AS RowNo FROM `indigo-lotus-154020.nodejs.quotation_updated_report` tb1) x WHERE x.RowNo = 1)'
